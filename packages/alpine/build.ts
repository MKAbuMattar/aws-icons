 ...
 * tree-shakeable index per style. Data feeds the Alpine directive.
 */
...
for (const style of STYLES) {
  ...
  for (const {slug, name, attrs, html} of eachIcon(ASSETS, style)) {
    ...
    fs.writeFileSync(
      ...
      `import type {AwsIconData} from '../../index.js';\nexport declare const ${name}: AwsIconData;\nexport default ${name};\n`,
    );
...
console.log(`@aws-icons/alpine: generated ${total} icon data modules`);
