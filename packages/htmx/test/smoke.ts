...
import {iconHtml, awsIconHandler} from '../src/index';

const svg = await iconHtml('rocket');
assert.ok(svg.startsWith('<svg'), 'serves an svg fragment');
assert.ok(svg.includes('aria-hidden="true"'), 'decorative by default');

const labeled = await iconHtml('rocket', 'modern', 'Rocket "1"');
assert.ok(
...
const ok = await call('/aws-icons/flat/rocket?label=Rocket');
...
const missing = await call('/aws-icons/flat/not-a-real-icon');
assert.equal(missing.statusCode, 404, 'unknown slug is 404');

const traversal = await call('/aws-icons/flat/..%2F..%2Fpackage');
