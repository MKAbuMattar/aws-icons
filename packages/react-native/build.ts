/**
 * Generates one icon module + d.ts per icon per set. Each module carries the
...
for (const style of STYLES) {
  ...
  for (const {slug, name} of eachIcon(ASSETS, style)) {
    ...
    fs.writeFileSync(
      ...
      `import {createAwsIcon} from '../../core.js';\nexport const ${name} = createAwsIcon(${JSON.stringify(name)}, ${JSON.stringify(xml)});\nexport default ${name};\n`,
    );
    fs.writeFileSync(
...
console.log(`@aws-icons/react-native: generated ${total} icon components`);
