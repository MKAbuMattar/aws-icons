 ...
 * module + d.ts per icon per style. Icon modules are plain JS importing the
...
export type AwsIconProps = JSX.SvgSVGAttributes<SVGSVGElement> & {title?: string};
export type AwsIconComponent = Component<AwsIconProps>;
export declare const createAwsIcon: (name: string, attrs: Record<string, string>, html: string) => AwsIconComponent;
`,
);
...
for (const style of STYLES) {
  ...
  for (const {slug, name, attrs, html} of eachIcon(ASSETS, style)) {
    ...
    fs.writeFileSync(
      ...
      `import {createAwsIcon} from '@aws-icons/solid';\nexport const ${name} = createAwsIcon(${args});\nexport default ${name};\n`,
    );
    fs.writeFileSync(
      path.join(iconsDir, `${slug}.d.ts`),
      `import type {AwsIconComponent} from '../../core.js';\nexport declare const ${name}: AwsIconComponent;\nexport default ${name};\n`,
    );
...
console.log(`@aws-icons/solid: generated ${total} icon components`);
