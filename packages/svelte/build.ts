 ...
 * tree-shakeable index per style. Data is rendered by src/Icon.svelte.
 */
...
for (const style of STYLES) {
  ...
  for (const {slug, name, attrs, html} of eachIcon(ASSETS, style)) {
    fs.writeFileSync(
      ...
      `export const ${name} = {attrs: ${JSON.stringify(attrs)}, html: ${JSON.stringify(html)}};\nexport default ${name};\n`,
    );
    fs.writeFileSync(
      path.join(iconsDir, `${slug}.d.ts`),
      `import type {AwsIconData} from '../../../src/index.js';\nexport declare const ${name}: AwsIconData;\nexport default ${name};\n`,
    );
...
console.log(`@aws-icons/svelte: generated ${total} icon data modules`);
